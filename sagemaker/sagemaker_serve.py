...
FastAPI server for SageMaker real-time endpoint.
Endpoints: GET /ping (health check) and POST /invocations (inference).
Port: 8080 (SageMaker requirement).
Model artifacts loaded from /opt/ml/model/ (SageMaker extracts model.tar.gz there).
"""

import sys
import os

# Import torch BEFORE modifying sys.path, so Python finds the real PyTorch
# (not transformers4rec/torch/ which would cause a circular import)
import torch
import numpy as np

# Point to local copies of transformers4rec and peft in the container
sys.path.insert(0, "/opt/program/transformers4rec")
sys.path.insert(0, "/opt/program/peft")
...
VOCAB_PATH = os.path.join(MODEL_DIR, "command_vocab.json")
AUGMENTATIONS_PATH = os.path.join(MODEL_DIR, "command_information_augmentations.csv")
METADATA_PATH = os.path.join(MODEL_DIR, "inference_metadata.json")
SCHEMA_DATASET_PATH = os.path.join(MODEL_DIR, "processed_nvt", "part_0.parquet")
MAX_SEQ_LEN = 110
TOP_K = 10
...
def load_model():
    ...
    # Numba CUDA patch — prevent segfault on driver mismatch
